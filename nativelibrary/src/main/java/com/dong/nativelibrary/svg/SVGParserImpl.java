...
import com.dong.nativeapp.BuildConfig;
import com.dong.nativeapp.svg.SVGBase;
...
class SVGParserImpl implements SVGParser
{
   ...
   private static class FontSizeKeywords {
      private static final Map<String, SVGBase.Length> fontSizeKeywords = new HashMap<>(9);
      static {
         fontSizeKeywords.put("xx-small", new SVGBase.Length(0.694f, SVGBase.Unit.pt));
         fontSizeKeywords.put("x-small", new SVGBase.Length(0.833f, SVGBase.Unit.pt));
         fontSizeKeywords.put("small", new SVGBase.Length(10.0f, SVGBase.Unit.pt));
         fontSizeKeywords.put("medium", new SVGBase.Length(12.0f, SVGBase.Unit.pt));
         fontSizeKeywords.put("large", new SVGBase.Length(14.4f, SVGBase.Unit.pt));
         fontSizeKeywords.put("x-large", new SVGBase.Length(17.3f, SVGBase.Unit.pt));
         fontSizeKeywords.put("xx-large", new SVGBase.Length(20.7f, SVGBase.Unit.pt));
         fontSizeKeywords.put("smaller", new SVGBase.Length(83.33f, SVGBase.Unit.percent));
         fontSizeKeywords.put("larger", new SVGBase.Length(120f, SVGBase.Unit.percent));
      }

      static SVGBase.Length get(String fontSize) {
   ...
   private void  endElement(String uri, String localName, String qName) throws SVGParseException
   {
      ...
      switch (SVGElem.fromString(tag))
      {
         ...
         case solidColor:
            ...
            currentElement = ((SVGBase.SvgObject) currentElement).parent;
            break;
   ...
   private void  parseAttributesTextPosition(SVGBase.TextPositionedContainer obj, Attributes attributes) throws SVGParseException
   {
      for (int i=0; i<attributes.getLength(); i++)
      {
         String val = attributes.getValue(i).trim();
   ...
   private void  tspan(Attributes attributes) throws SVGParseException
   {
      ...
      currentElement.addChild(obj);
      currentElement = obj;
      if (obj.parent instanceof SVGBase.TextRoot)
         obj.setTextRoot((SVGBase.TextRoot) obj.parent);
      else
         obj.setTextRoot(((SVGBase.TextChild) obj.parent).getTextRoot());
   }
      ...
      for (int i=0; i<attributes.getLength(); i++)
      {
         ...
         switch (SVGAttr.fromString(attributes.getLocalName(i)))
         {
            ...
            case spreadMethod:
               try
               {
                  obj.spreadMethod = SVGBase.GradientSpread.valueOf(val);
               } 
   ...
   private void  parseAttributesCore(SVGBase.SvgElementBase obj, Attributes attributes) throws SVGParseException
   {
      for (int i=0; i<attributes.getLength(); i++)
      {
    ...
    */
   private void  parseAttributesStyle(SVGBase.SvgElementBase obj, Attributes attributes)
   {
      for (int i=0; i<attributes.getLength(); i++)
      {
         String  val = attributes.getValue(i).trim();
    ...
    */
   private static void  parseStyle(SVGBase.SvgElementBase obj, String style)
   {
    ...
    */
   static SVGBase.Length parseLength(String val) throws SVGParseException
   {
      if (val.length() == 0)
      ...
      if (lastChar == '%') {
         ...
         try {
            unit = SVGBase.Unit.valueOf(unitStr.toLowerCase(Locale.US));
         } catch (IllegalArgumentException e) {
    ...
    */
   private static List<SVGBase.Length>  parseLengthList(String val) throws SVGParseException
   {
      if (val.length() == 0)
         throw new SVGParseException("Invalid length list (empty string)");

      List<SVGBase.Length>  coords = new ArrayList<>(1);
      ...
      while (!scan.empty())
      {
         ...
         SVGBase.Unit unit = scan.nextUnit();
         if (unit == null)
            unit = SVGBase.Unit.px;
         coords.add(new SVGBase.Length(scalar, unit));
         scan.skipCommaWhitespace();
      }
      return coords;
   }
    ...
    */
   private static SVGBase.Box parseViewBox(String val) throws SVGParseException
   {
      TextScanner scan = new TextScanner(val);
      scan.skipWhitespace();
    ...
    */
   static SVGBase.SvgPaint parsePaintSpecifier(String val)
   {
      if (val.startsWith("url("))
      {
         ...
         if (closeBracket != -1)
         {
            ...
            SVGBase.SvgPaint fallback = null;

            val = val.substring(closeBracket+1).trim();
            if (val.length() > 0)
               fallback = parseColourSpecifer(val);
            return new SVGBase.PaintReference(href, fallback);
         }
         else
         {
            String    href = val.substring(4).trim();
            return new SVGBase.PaintReference(href, null);
         }
      }
   ...
   private static SVGBase.SvgPaint parseColourSpecifer(String val)
   {
      switch (val) {
         case NONE:
    ...
    */
   static SVGBase.Colour parseColour(String val)
   {
      if (val.charAt(0) == '#')
      {
         ...
         switch (pos) {
            case 4:
               ...
               h2 = threehex & 0x0f0;  // g
               h3 = threehex & 0x00f;  // b
               return new SVGBase.Colour(0xff000000|h1<<12|h1<<8|h2<<8|h2<<4|h3<<4|h3);
            case 5:
               ...
               return new SVGBase.Colour(h4<<28|h4<<24 | h1<<8|h1<<4 | h2<<4|h2 | h3|h3>>4);
            case 7:
               return new SVGBase.Colour(0xff000000 | ip.value());
            case 9:
               return new SVGBase.Colour(ip.value() << 24 | ip.value() >>> 8);
            default:
            ...
            if (!Float.isNaN(green)) {
               ...
               if (isLegacyCSSColor3) {
                  if (!scan.skipCommaWhitespace())
                     return SVGBase.Colour.BLACK;   // Error
               } else {
                  scan.skipWhitespace();
               }
               ...
               if (!Float.isNaN(blue)) {
                  ...
                  if (Float.isNaN(alpha))
                     return new SVGBase.Colour( 0xff000000 | clamp255(red)<<16 | clamp255(green)<<8 | clamp255(blue) );
                  else
                     return new SVGBase.Colour( clamp255(alpha * 256)<<24 | clamp255(red)<<16 | clamp255(green)<<8 | clamp255(blue) );
               }
            }
         }
      }
               ...
               if (!Float.isNaN(saturation)) {
                  ...
                  if (!Float.isNaN(lightness)) {
                     ...
                     if (!scan.consume(')'))
                        return SVGBase.Colour.BLACK;
                     if (Float.isNaN(alpha))
                        return new SVGBase.Colour( 0xff000000 | hslToRgb(hue, saturation, lightness) );
                     else
                        return new SVGBase.Colour( clamp255(alpha * 256)<<24 | hslToRgb(hue, saturation, lightness) );
                  }
               }
            }
         }
   ...
   private static SVGBase.Colour parseColourKeyword(String nameLowerCase)
   {
      Integer  col = ColourKeywords.get(nameLowerCase);
      return (col == null) ? SVGBase.Colour.BLACK :new SVGBase.Colour(col);
   }
   ...
   static void  parseFont(Style style, String val)
   {
      ...
      SVGBase.Length fontSize = parseFontSize(item);
   ...
   static SVGBase.Length parseFontSize(String val)
   {
      try {
         SVGBase.Length size = FontSizeKeywords.get(val);
         if (size == null)
   ...
   static Style.TextDecoration parseTextDecoration(String val)
   {
      switch (val)
      {
   ...
   // Parse a text decoration keyword
   static Style.TextDirection parseTextDirection(String val)
   {
      switch (val)
      {
   ...
   static SVGBase.Length[]  parseStrokeDashArray(String val)
   {
      TextScanner scan = new TextScanner(val);
      scan.skipWhitespace();
      ...
      SVGBase.Length dash = scan.nextLength();
      if (dash == null)
         return null;
      if (dash.isNegative())
         return null;
      ...
      List<SVGBase.Length> dashes = new ArrayList<>();
      dashes.add(dash);
      while (!scan.empty())
      {
         scan.skipCommaWhitespace();
      ...
      return dashes.toArray(new SVGBase.Length[0]);
   }
   ...
   static SVGBase.CSSClipRect parseClip(String val)
   {
      if ("auto".equals(val))
         return null;
      ...
      SVGBase.Length top = parseLengthOrAuto(scan);
      scan.skipCommaWhitespace();
      SVGBase.Length right = parseLengthOrAuto(scan);
      scan.skipCommaWhitespace();
      SVGBase.Length bottom = parseLengthOrAuto(scan);
      scan.skipCommaWhitespace();
      SVGBase.Length left = parseLengthOrAuto(scan);

      scan.skipWhitespace();
      if (!scan.consume(')') && !scan.empty())   // Be forgibing. Allow missing ')'.
         return null;

      return new SVGBase.CSSClipRect(top, right, bottom, left);
   }


   private static SVGBase.Length parseLengthOrAuto(TextScanner scan)
   {
   ...
   static Style.VectorEffect parseVectorEffect(String val)
   {
      switch (val)
      {
   ...
   static Style.RenderQuality parseRenderQuality(String val)
   {
      switch (val)
      {
   ...
   static Style.Isolation parseIsolation(String val)
   {
      switch (val)
      {
   ...
   static SVGBase.Length parseLetterOrWordSpacing(String val)
   {
      if ("normal".equals(val))
         return SVGBase.Length.ZERO;
      else {
         try {
            SVGBase.Length result = parseLength(val);
            // Percent units were removed in SVG2 and are treated as an error.
            return (result.unit == SVGBase.Unit.percent) ? null : result;
         } catch (SVGParseException e) {
            return null;
         }
      }
   ...
   {
      CSSParser  cssp = new CSSParser(CSSParser.MediaType.screen, CSSParser.Source.Document, externalFileResolver);
